...
下面是这个包里 `preinst` 的核心限制逻辑。问题在于它只看版本号，没有先确认当前发行版是否真的是 Ubuntu：
...
在 Debian 13 上，`VERSION_ID=13`，因此会被错误地当成“小于 18.04”，安装直接失败。
...
- Ubuntu：仍然保留原始的 `18.04+` 要求
- Debian 等非 Ubuntu：不再被这条 Ubuntu 专属判断错误拦截
